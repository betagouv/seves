from functools import cached_property
import json

from django.forms import Media
from django.forms.models import BaseModelFormSet, modelformset_factory

from core.form_mixins import js_module
from core.forms import DocumentUploadForm
from core.models import Document
from core.validators import MAX_UPLOAD_SIZE_MEGABYTES, AllowedExtensions


class DocumentUploadBaseFormSet(BaseModelFormSet):
    template_name = "core/form/document_uploads.html"

    @property
    def media(self):
        return super().media + Media(css={"all": ("core/form/document_uploads.css",)})

    @property
    def max_upload_size_mb(self):
        return MAX_UPLOAD_SIZE_MEGABYTES

    @property
    def allowed_extensions(self):
        return AllowedExtensions.values

    @cached_property
    def allowed_extensions_per_document_type(self):
        result = Document.get_accept_attribute_per_document_type()
        result[""] = "." + ",.".join(self.allowed_extensions)
        return json.dumps(result)

    def __init__(
        self,
        user,
        related_to,
        allowed_document_types,
        next_url,
        data=None,
        files=None,
        auto_id="id_%s",
        prefix=None,
        queryset=None,
        *,
        initial=None,
        **kwargs,
    ):
        self.allowed_document_types = allowed_document_types
        self.user = user
        self.related_to = related_to
        self.next_url = next_url
        super().__init__(data, files, auto_id, prefix, queryset, initial=initial, **kwargs)

    def add_prefix(self, index):
        """Form prefixing is disabled as subforms will be processed individually"""
        return ""

    def get_form_kwargs(self, index):
        return {
            **super().get_form_kwargs(index),
            "user": self.user,
            "related_to": self.related_to,
            "allowed_document_types": self.allowed_document_types,
        }

    def get_queryset(self):
        return self.queryset or super().get_queryset().none()


class FicheDocumentUploadBaseFormSet(DocumentUploadBaseFormSet):
    @property
    def media(self):
        return super().media + Media(js=(js_module("core/form/fiche_document_uploads.mjs"),))


class MessageDocumentUploadBaseFormSet(DocumentUploadBaseFormSet):
    @property
    def media(self):
        return super().media + Media(js=(js_module("core/form/message_document_uploads.mjs"),))


FicheDocumentUploadFormSet = modelformset_factory(
    Document, DocumentUploadForm, formset=FicheDocumentUploadBaseFormSet, extra=0
)

MessageDocumentUploadFormSet = modelformset_factory(
    Document, DocumentUploadForm, formset=MessageDocumentUploadBaseFormSet, extra=0
)
