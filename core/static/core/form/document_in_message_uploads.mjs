 ...
 */
class FileMeta {
    constructor({file, title, validated}) {
        this.file = file
        this.title = title || ""
...
class DocumentValidated extends Controller {
    ...
    onDelete() {
        this.setFilesValue(value => {
            delete value[this.fileIdValue]
            return {...value}
        })
    }
...
class DocumentForm extends Controller {
    ...
    /** @param {FileStoreStruct} files */
    onFilesUpdate(files) {
        if(this.initialValue === false && !Object.hasOwn(files, this.fileIdValue)) {
            this.element.remove()
        }
    }
