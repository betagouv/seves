...
def generic_test_can_filter_by_fin_de_suivi(live_server, page: Page, factory, url, mocked_authentification_user):
    ...
    page.get_by_test_id("submit-search").click(force=True)

    expect(page.get_by_role("cell", name=str(obj_1.numero), exact=True)).not_to_be_visible()
    expect(page.get_by_role("cell", name=str(obj_2.numero), exact=True)).not_to_be_visible()
    expect(page.get_by_role("cell", name=str(obj_3.numero), exact=True)).not_to_be_visible()
    expect(page.get_by_role("cell", name=str(obj_4.numero), exact=True)).to_be_visible()
    expect(page.get_by_role("cell", name=str(obj_5.numero), exact=True)).to_be_visible()
    expect(page.get_by_role("cell", name=str(obj_6.numero), exact=True)).to_be_visible()
