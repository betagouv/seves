...
logger = getLogger("ssa.migrations.0062_etablissement_code_postal")
...
def populate_commmunes(apps, _):
    ...
    count = (
        Etablissement.objects.filter(code_postal="")
        .annotate(ADRE_CP=KT("historical_data__ADRE_CP"))
        .exclude(ADRE_CP="")
        .update(code_postal=F("ADRE_CP"))
    )

    logging.info(f"Updated {count} records from historical_data__ADRE_CP")
