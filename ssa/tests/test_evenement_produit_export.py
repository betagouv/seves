...
def test_export_evenement_produit_simple_case(mailoutbox):
    ...
    assert (
        lines[0]
        == '"Année","Numéro","Structure créatrice","Date de création","Numéro RASFF","Type d\'événement","Source","Description","Catégorie de produit","Dénomination","Marque","Lots, DLC/DDM","Description complémentaire","Température de conservation","Catégorie de danger","Quantification","Unité de quantification","Évaluation","Produit prêt a manger","Référence souches","Référence clusters","Actions engagées","Numéro de rappels conso","Numéro SIRET","Raison sociale","Adresse ou lieu-dit","Commune","Département","Pays établissement","Type d\'exploitant","Position dans le dossier"\r'
    )

    expected_fields = [
        str(evenement.numero_annee),
        str(evenement.numero_evenement),
        str(evenement.createur),
        str(evenement.date_creation),
    ...
    assert mail.subject == "Votre export est prêt"
...
def test_export_evenement_produit_content_etablissement(mailoutbox):
    ...
    assert expected_fields == next(csv.reader(StringIO(lines[1])))[23:31]
    ...
    assert expected_fields == next(csv.reader(StringIO(lines[2])))[23:31]
    assert lines[3] == ""
    assert len(lines) == 4
