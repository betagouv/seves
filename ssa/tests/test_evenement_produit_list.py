...
def test_list_filter_with_free_search_changes_ordering(live_server, mocked_authentification_user, page: Page):
    evenement_1 = EvenementProduitFactory(description_complementaire="Morbier")
    evenement_2 = EvenementProduitFactory(description="Morbier")
    evenement_3 = EvenementProduitFactory(denomination="Morbier")
    search_page = EvenementProduitListPage(page, live_server.url)
    search_page.navigate()
    ...
    expect(search_page.numero_cell(line_index=1)).to_have_text(evenement_2.numero)
    expect(search_page.numero_cell(line_index=2)).to_have_text(evenement_3.numero)
    expect(search_page.numero_cell(line_index=3)).to_have_text(evenement_1.numero)
