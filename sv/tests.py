...
class FicheDetectionUpdateViewTests(StaticLiveServerTestCase):
	...
	def setUp(self):
		...
		self.edit_fiche_detection_url = f'{self.live_server_url}/sv/fiches-detection/{self.fiche_detection.id}/modification/'
	...
	def test_fiche_detection_update_page_content(self):
		...
		expect(page.locator("#organisme-nuisible")).to_contain_text(self.fiche_detection.organisme_nuisible.libelle_court)
		expect(page.get_by_label("Statut règlementaire")).to_contain_text(self.fiche_detection.statut_reglementaire.libelle)
		...
		expect(page.get_by_label("Mesures conservatoires immédiates")).to_have_value(self.fiche_detection.mesures_conservatoires_immediates)
		...
		expect(page.get_by_label("Mesures de surveillance spécifique")).to_have_value(self.fiche_detection.mesures_surveillance_specifique)
		context.close()
		page.close()
	...
	def test_fiche_detection_update_with_createur_only(self):
		...
		fiche_detection = FicheDetection.objects.create(
			numero=NumeroFiche.get_next_numero(),
			createur=Unite.objects.create(nom='Mission des urgences sanitaires', type=Administration.objects.create(nom='AC')),
		)
	...
	def test_fiche_detection_update_without_lieux_and_prelevement(self):
		...
		self.assertEqual(fiche_detection.mesures_conservatoires_immediates, new_mesures_conservatoires_immediates)
		...
		self.assertEqual(fiche_detection.mesures_surveillance_specifique, new_mesures_surveillance_specifique)
		context.close()
		page.close()
