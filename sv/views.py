...
class FicheDetectionUpdateView(FicheDetectionContextMixin, UpdateView):
    ...
    def allows_inactive_laboratoires_agrees_values(self):
        actual_ids = Prelevement.objects.filter(lieu__fiche_detection__pk=self.object.pk).values_list("laboratoire_agree_id", flat=True)
    ...
    def allows_inactive_laboratoires_confirmation_values(self):
        actual_ids = Prelevement.objects.filter(lieu__fiche_detection__pk=self.object.pk).values_list("laboratoire_confirmation_officielle_id", flat=True)
        inactive_ids = LaboratoireConfirmationOfficielle._base_manager.filter(is_active=False).values_list("id", flat=True)
        return any([pk in inactive_ids for pk in actual_ids if pk])
