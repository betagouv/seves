...
def test_cant_click_on_shortcut_when_no_structure(live_server, page: Page):
    ...
    expect(message_page.page.get_by_role("link", name="Ajouter toutes les structures de la fiche")).not_to_be_visible()
    expect(message_page.page.get_by_role("link", name="Ajouter tous les contacts de la fiche")).not_to_be_visible()
...
def test_can_click_on_add_all_contacts_shortcut_when_evenement_has_contact(
    ...
    evenement = EvenementFactory()
    contact_structure = ContactStructureFactory(with_one_active_agent__with_groups=(settings.SV_GROUP,))
...
def test_can_add_draft_demande_intervention(
    live_server, page: Page, choice_js_fill, mailoutbox, mocked_authentification_user
):
    ...
    agent.save()
    active_contact = ContactStructureFactory(with_one_active_agent__with_groups=(settings.SSA_GROUP, settings.SV_GROUP))
    evenement = EvenementFactory()
...
def test_contact_shorcut_excludes_agent_and_structures_in_fin_suivi(live_server, page: Page, choice_js_get_values):
    generic_test_contact_shorcut_excludes_agent_and_structures_in_fin_suivi(
        live_server, page, choice_js_get_values, EvenementFactory()
    )


def test_can_search_in_message_list(live_server, page: Page):
    generic_test_can_search_in_message_list(live_server, page, EvenementFactory())


def test_choices_js_only_search_in_label(live_server, page: Page, choice_js_get_all_values):
    evenement = EvenementFactory()
    page.goto(f"{live_server.url}{evenement.get_absolute_url()}")

    ContactStructureFactory(structure__libelle="Structure 11", structure__force_can_be_contacted=True)
    ContactStructureFactory(structure__libelle="Ma structure", id=11, structure__force_can_be_contacted=True)
    page.get_by_test_id("element-actions").click()
    ...
    assert "Ma structure" in results
