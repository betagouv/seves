...
def test_cloture_evenement_auto_fin_suivi_si_derniere_structure_ac(
    ...
    FinSuiviContact(
        content_type=ContentType.objects.get_for_model(evenement), object_id=evenement.id, contact=contact_1
    ).save()
    ...
    assert FinSuiviContact.objects.filter(
        content_type=ContentType.objects.get_for_model(evenement), object_id=evenement.id, contact=contact_mus
    ).exists()
    assert Message.objects.filter(
        message_type=Message.FIN_SUIVI, sender=mocked_authentification_user.agent.contact_set.get()
    ).exists()
