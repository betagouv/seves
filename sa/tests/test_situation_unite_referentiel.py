...
from sa.tests.factories import EspeceConcerneeFactory, EspeceFactory, SituationUniteRegleFactory
...
@pytest.mark.django_db
def test_saving_a_situation_unite_regle_for_the_wrong_espece_outside_the_form_is_rejected():
    espece = EspeceFactory()
    other_espece = EspeceFactory()
    regle = SituationUniteRegleFactory(espece=other_espece, type_lieu="Élevage")

    espece_concernee = EspeceConcerneeFactory.build(espece=espece, situation_unite_regle=regle)
...
def test_saving_a_valid_combination_outside_the_form_succeeds():
    espece = EspeceFactory()
    regle = SituationUniteRegleFactory(
        espece=espece,
        type_lieu="Élevage",
        mode_elevage="Bâtiment ouvert (en partie)",
        type_production="Atelier laitier",
    )
    espece_concernee = EspeceConcerneeFactory.create(espece=espece, situation_unite_regle=regle)
