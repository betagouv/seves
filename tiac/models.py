...
class EvenementSimple(
    ...
    date_reception = models.DateTimeField(auto_now_add=True, verbose_name="Date de réception à la DD(ETS)PP")
    ...
    modalites_declaration = models.IntegerField(
        choices=ModaliteDeclarationEvenement.choices, verbose_name="Modalités de déclaration"
    )
...
class EvenementsLies(models.Model):
    evenement = models.ForeignKey(EvenementSimple, on_delete=models.CASCADE, related_name="+")
    releted_evenements_ct = models.ForeignKey(ContentType, on_delete=models.CASCADE, related_name="+")
    releted_evenements_id = models.BigIntegerField()
    evenements_lies = GenericForeignKey("releted_evenements_ct", "releted_evenements_id")
