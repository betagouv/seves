...
from tiac.constants import EvenementFollowUp
...
def test_can_view_evenement_simple_history(live_server, page, mus_contact):
    evenement = EvenementSimpleFactory(follow_up=EvenementFollowUp.NONE)
    ...
    expect(page.locator("tr")).to_have_count(
        len(
            [
                ...
                "One for the change of follow up value (due to transformation into Investigation)",
    ...
    expect(page.get_by_text("Structure Test")).to_have_count(
        len(
            [
                "Cloturé for transformation",
                "Follow up changed for transformation",
                "Lien libre for transformation",
