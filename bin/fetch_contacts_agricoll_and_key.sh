...
sshpass -p "$SFTP_PASSWORD" sftp $SFTP_OPTIONS "$SFTP_CONNECT:$LATEST_ENCRYPTED_KEY_FILENAME" .
sshpass -p "$SFTP_PASSWORD" sftp $SFTP_OPTIONS "$SFTP_CONNECT:$LATEST_ENCRYPTED_DATA_FILENAME" .

# Déchiffre la clé symétrique
echo $SFTP_PRIVATE_KEY | base64 --decode > $PRIVATE_KEY_FILENAME
...
rm -f clevercloud_known_hosts \
      ...
      $LATEST_ENCRYPTED_KEY_FILENAME \
      $LATEST_ENCRYPTED_DATA_FILENAME \
